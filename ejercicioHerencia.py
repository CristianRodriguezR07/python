...
def generar_informe():
    print("**Información de cada persona:**")
    for persona in Persona.__subclasses__():
        if persona.total_personas > 0:
            print(f"- {persona.__name__} ({persona.total_personas})")
            for obj in persona.__dict__.values():
                if isinstance(obj, list):
                    for p in obj:
                        p.imprimir_informacion()
                else:
                    if hasattr(obj, "imprimir_informacion"):
                        obj.imprimir_informacion()
